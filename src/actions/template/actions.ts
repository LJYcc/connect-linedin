 ...
 * @returns
 */
export const getConnections = (networkHelper: NetworkHelper) => async (page: Page) => {
...
export const sendMessage =
  (networkHelper: NetworkHelper, recipient: string, message: string) => async (page: Page) => {
...
export const sendConnectionRequest =
  (networkHelper: NetworkHelper, personName: string, message?: string) => async (page: Page) => {
    try {
      await searchOnHomePage(page, networkHelper, personName)
      await clickFirstResultOnSearchPage('People', page, networkHelper)
 ...
 */
export async function searchOnHomePage(page: Page, networkHelper: NetworkHelper, query: string) {
...
export const clickConnectButtonOnPersonPage = async (page: Page, networkHelper: NetworkHelper) => {
  await networkHelper.waitForPageLoad(page)
...
export const fulfillConnectionRequestOnConnectionModal = async (
  ...
  const addNoteButton = await page.locator('button:has-text("Add a note")').first()
 ...
 * @returns
 */
export async function readProfileIntroOnPersonPage(page: Page, networkHelper: NetworkHelper) {
 ...
 * @returns
 */
export async function readContactInfoOnContactModal(page: Page, networkHelper: NetworkHelper) {
  return await networkHelper.retryWithBackoff(async () => {
    await networkHelper.waitForNetworkIdle(page)
...
export async function followCompanyOnCompanyPage(
  ...
  if (
    followButtonText &&
    (followButtonText.trim() === 'Follow' || followButtonText.trim() === '+ Follow')
  ) {
    await networkHelper.retryWithBackoff(async () => {
 ...
 */
export const clickOnPostModal = async (networkHelper: NetworkHelper, page: Page) => {
 ...
 */
export const clickPostButton = async (networkHelper: NetworkHelper, page: Page) => {
 ...
 */
export const navigateToProfile = async (networkHelper: NetworkHelper, page: Page) => {
  // Wait for all network activity to settle
  await networkHelper.waitForNetworkIdle(page)
...
export const readContactsFromConnectionPage = async (
  ...
  return await page.$$eval('.mn-connection-card', cards =>
    cards.map(card => ({
      ...
      title: card.querySelector('.mn-connection-card__occupation')?.textContent?.trim() || '',
      profileUrl: (card.querySelector('.mn-connection-card__link') as HTMLAnchorElement)?.href || ''
    }))
  )
}
 ...
 */
export const navigateToConnections = async (networkHelper: NetworkHelper, page: Page) => {
